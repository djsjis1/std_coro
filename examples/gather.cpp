...
coro::Task<> main_task() {
    std::cout << "=== Gather Example ===" << std::endl << std::endl;
    ...
    auto [r1, r2, r3] = co_await coro::gather(api_call("users", 300), api_call("posts", 200), api_call("photos", 100));
    std::cout << "   results: " << r1 << " " << r2 << " " << r3 << std::endl;
    std::cout << "   (total ~300ms, not 300+200+100=600ms)" << std::endl << std::endl;

    // 2. Mix string and int return types
    std::cout << "2. Mixed string and int types:" << std::endl;
    auto [name, value] = co_await coro::gather(api_call("status", 250), risky_call(42, 350, false));
    std::cout << "   name=" << name << ", value=" << value << std::endl << std::endl;

    // 3. Exception handling: one task fails, wait for others, rethrow
    std::cout << "3. Exception handling (one task fails):" << std::endl;
    try {
        auto [a, b, c] = co_await coro::gather(risky_call(1, 100, false), risky_call(2, 200, true), // this one throws
    ...
    std::cout << std::endl << "=== Gather example done ===" << std::endl;
}
