...
coro::Task<std::string> download_async(AsyncDownloader &dl,
                                       const std::string &url)
{
    ...
    // 在回调中完成 Promise
    dl.download(url, [&promise](const std::string &result)
...
coro::Task<> main_task()
{
    ...
    std::cout << "   val(42) * 10 = " << val << std::endl
              << std::endl;

    // 3. 多个 Promise 并发 gather
    std::cout << "3. Multiple Promises with gather:" << std::endl;
    auto [v1, v2] = co_await coro::gather(
        delayed_compute(10, 300),
        delayed_compute(20, 200));
    std::cout << "   v1=" << v1 << ", v2=" << v2 << std::endl;
    std::cout << "   (total ~300ms)" << std::endl
              << std::endl;
    ...
    std::cout << std::endl
              << "=== Future/Promise example done ===" << std::endl;
}
