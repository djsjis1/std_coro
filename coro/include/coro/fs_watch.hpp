...
namespace coro
{
    namespace fs
    {
        ...
        public:
            ...
            explicit DirectoryWatcher(HANDLE dir, bool recursive)
                : dir_(dir), recursive_(recursive)
            {
                if (valid())
            ...
            struct changes_awaiter
            {
                ...
                void await_suspend(std::coroutine_handle<> h)
                {
                    ...
                    BOOL rc = ReadDirectoryChangesW(w->dir_, buf, sizeof(buf),
                                                    w->recursive_, filter,
                                                    nullptr /*完成时字节数经完成包给*/,
                                                    &op.ov, nullptr);
                ...
                void await_resume()
                {
                    w->parse_records(buf, op.error, op.transferred);
                }
            };
        ...
        private:
            ...
            void parse_records(const char *buf, int error, DWORD bytes)
            {
                ...
                while (true)
                {
                    ...
                    std::string name = detail_watch::wide_to_utf8(
                        ni->FileName, ni->FileNameLength / sizeof(WCHAR));
                    watch_event ev;
                    bool has_event = true;
                    switch (ni->Action)
                    {
                ...
                if (!rename_old.empty())
                    pending_.push_back(
                        watch_event{watch_event_type::removed, std::move(rename_old), "", false});
            }
        };
        ...
        public:
            DirectoryWatcher() = default;
            explicit DirectoryWatcher(int fd, bool recursive)
                : fd_(fd), recursive_(recursive) {}
            ~DirectoryWatcher() { close(); }
        ...
        private:
            ...
            void parse_records(const char *buf, int bytes)
            {
                ...
                while (p + sizeof(struct inotify_event) <= end)
                {
                    ...
                    {
                        pending_.push_back(
                            watch_event{watch_event_type::overflow, "", "", false});
                        continue;
                    }
                    if (ie->mask & IN_CREATE)
                    {
                        pending_.push_back(
                            watch_event{watch_event_type::created, std::move(name), "", is_dir});
                        // 递归: 新子目录 → 追加 watch
                    ...
                    {
                        pending_.push_back(
                            watch_event{watch_event_type::removed, std::move(name), "", is_dir});
                        continue;
                    }
                    if (ie->mask & IN_MODIFY || ie->mask & IN_ATTRIB)
                    {
                        pending_.push_back(
                            watch_event{watch_event_type::modified, std::move(name), "", is_dir});
                        continue;
                    }
                    ...
                    if (ie->mask & IN_MOVED_TO)
                    {
                        ...
                        {
                            pending_.push_back(watch_event{watch_event_type::renamed,
                                                           std::move(name),
                                                           std::move(rename_old), is_dir});
                            rename_cookie = 0;
                        }
                        else
                        {
