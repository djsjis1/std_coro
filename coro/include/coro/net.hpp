    ...
    namespace net
    {
        ...
        public:
            ...
            struct connect_awaiter
            {
                ...
                void await_suspend(std::coroutine_handle<> h)
                {
                    ...
                    BOOL rc = connect_ex_(sock, (sockaddr *)&addr, sizeof(addr),
        ...
        public:
            ...
            bool bind_listen(const char *ip, unsigned short port)
            {
                ...
                DWORD bytes = 0;
                WSAIoctl(sock_, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid,
                         sizeof(guid), &accept_ex_, sizeof(accept_ex_),
            ...
            struct accept_awaiter
            {
                ...
                void await_suspend(std::coroutine_handle<> h)
                {
                    ...
                    DWORD received = 0;
                    BOOL rc = listener->accept_ex_(listener->sock_, accepted,
                                                   addr_buf, 0, ADDR_LEN, ADDR_LEN,
                                                   &received, &op.ov);
                    if (rc)
                    {
