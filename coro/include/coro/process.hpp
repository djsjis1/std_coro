...
namespace coro
{
    namespace process
    {
        ...
        inline Task<Process> Process::create(std::vector<std::string> args, options opt)
        {
            ...
            Process out;
                ...
                auto make_pipe = [&](bool capture, DWORD server_access, DWORD client_access,
                                     std::optional<pipe::PipeEnd> *parent_side,
                                     HANDLE *child_side) -> bool
                {
                    if (!capture)
                        return true;
                    ...
                    std::snprintf(name, sizeof(name), "\\\\.\\pipe\\coro_proc_%lu_%llu",
                                  ...
                                  (unsigned long long)n.fetch_add(1));
                    std::wstring wname(name, name + std::strlen(name));
                    HANDLE server = CreateNamedPipeW(
                        wname.c_str(), server_access | FILE_FLAG_OVERLAPPED,
                    ...
                    HANDLE client = CreateFileW(wname.c_str(), client_access, 0,
                ...
                if (!make_pipe(opt.capture_stdin, PIPE_ACCESS_OUTBOUND, GENERIC_READ,
                               &out.stdin_, &child_stdin) ||
                    !make_pipe(opt.capture_stdout, PIPE_ACCESS_INBOUND, GENERIC_WRITE,
                               &out.stdout_, &child_stdout) ||
                    !make_pipe(opt.capture_stderr, PIPE_ACCESS_INBOUND, GENERIC_WRITE,
                               &out.stderr_, &child_stderr))
                {
                ...
                // 未捕获的 stdio: 尽量继承控制台句柄 (无控制台则给 NUL)
                HANDLE fallback = CreateFileW(L"NUL", GENERIC_READ | GENERIC_WRITE,
                ...
                auto ctx = new std::pair<HANDLE, std::shared_ptr<Promise<int>>>(
                    pi.hProcess, std::move(state));
                RegisterWaitForSingleObject(
                    &out.wait_registration_, pi.hProcess,
                    [](void *p, BOOLEAN)
                    {
                        auto *c = static_cast<std::pair<HANDLE, std::shared_ptr<Promise<int>>> *>(p);
                        DWORD code = 255;
                        GetExitCodeProcess(c->first, &code);
                        c->second->set_value((int)code); // 跨线程 → 等待者 loop
                        delete c;
                    },
                    ctx, INFINITE, WT_EXECUTEONLYONCE | WT_EXECUTEINWAITTHREAD);
        ...
        // 设计: 不碰全局信号掩码 (避开 signal.hpp 的 signalfd 集成),
        ...
        inline Task<Process> spawn(std::vector<std::string> args, options opt = {})
        {
            ...
            {
                for (int f : {in_pipe[0], in_pipe[1], out_pipe[0], out_pipe[1],
        ...
        inline Task<std::pair<int, std::string>> run_capture(
