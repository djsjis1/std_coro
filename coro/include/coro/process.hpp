...
namespace coro {
    namespace process {
        ...
        inline Task<Process> Process::create(std::vector<std::string> args, options opt) {
            ...
            HANDLE fallback = CreateFileW(L"NUL", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
            ...
            BOOL ok = CreateProcessW(nullptr, cmd_buf.data(), nullptr, nullptr, TRUE, 0, nullptr, nullptr, &si, &pi);
            for (HANDLE h : {child_stdin, child_stdout, child_stderr})
            ...
            auto ctx = new std::pair<HANDLE, std::shared_ptr<Promise<int>>>(pi.hProcess, std::move(state));
