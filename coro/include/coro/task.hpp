...
namespace coro
{
    ...
    namespace detail
    {
        ...
        struct final_awaiter
        {
            ...
            bool await_suspend(std::coroutine_handle<Promise> h) noexcept
            {
                ...
                {
                    EventLoop &loop = promise.continuation_loop_
                                          ? *promise.continuation_loop_
                                          : EventLoop::get();
                ...
                {
                    EventLoop &loop = promise.target_loop_
                                          ? *promise.target_loop_
                                          : EventLoop::get();
        ...
        struct has_cancel_op : std::false_type
        {
        };
        template <typename T>
        ...
        template <typename T, typename = void>
        struct has_on_waiter_destroyed : std::false_type
        {
        };
        template <typename T>
        struct has_on_waiter_destroyed<T, std::void_t<decltype(std::declval<T &>().on_waiter_destroyed(std::declval<std::coroutine_handle<>>()))>> : std::true_type
        {
        };

        template <typename T>
        void notify_waiter_destroyed(T &inner, std::coroutine_handle<> h)
        {
        ...
        struct cancel_check_awaiter
        {
            using stored_t = std::conditional_t<std::is_lvalue_reference_v<Awaiter>,
                                                Awaiter,
                                                std::remove_reference_t<Awaiter>>;
            ...
            void await_suspend(std::coroutine_handle<Promise> h)
            {
                ...
                {
                    promise->cancel_hook_ = [](void *self)
                    { inner_t::cancel_op(self); };
    ...
    public:
        ...
        struct promise_type
        {
            ...
            {
                return detail::cancel_check_awaiter<promise_type, Awaiter &&>(
                    std::forward<Awaiter>(awaiter), this, &cancelled_);
            }
        ...
        Task(Task &&other) noexcept
            : handle_(std::exchange(other.handle_, nullptr)), result_(std::move(other.result_)), exception_(std::move(other.exception_)), ready_(other.ready_), started_(other.started_)
        {
            if (handle_)
            {
        ...
        void await_suspend(std::coroutine_handle<> continuation)
        {
            ...
            {
                started_ = true;
                EventLoop &loop = handle_.promise().target_loop_
                                      ? *handle_.promise().target_loop_
                                      : EventLoop::get();
    ...
    public:
        ...
        Task(Task &&other) noexcept
            : handle_(std::exchange(other.handle_, nullptr)), exception_(std::move(other.exception_)), ready_(other.ready_), started_(other.started_)
        {
            if (handle_)
                handle_.promise().task_ = this;
        }
