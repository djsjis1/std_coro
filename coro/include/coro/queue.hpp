...
namespace coro
{
    ...
    public:
        ...
        public:
            ...
            {
                return !q_->items_.empty();
            }

            /// 没数据 → 把协程句柄登记到等待队列, 然后挂起
            void await_suspend(std::coroutine_handle<> h)
            {
                q_->get_waiters_.push_back(h);
            }
        ...
        public:
            ...
            bool await_ready() const noexcept
            {
                return q_->maxsize_ == 0 || q_->items_.size() < q_->maxsize_;
            }

            /// 满了 → 把协程句柄登记到等待队列, 然后挂起
            void await_suspend(std::coroutine_handle<> h)
            {
                q_->put_waiters_.push_back(h);
            }
        ...
        struct join_awaiter
        {
            ...
            bool await_ready() const noexcept
            {
                return q->unfinished_tasks_ == 0;
            }
            void await_suspend(std::coroutine_handle<> h)
            {
                q->join_waiters_.push_back(h);
            }
