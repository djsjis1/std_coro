    ...
    namespace pipe {
        ...
        class PipeEnd {
          public:
            ...
            PipeEnd(PipeEnd&& other) noexcept : handle_(std::exchange(other.handle_, INVALID_HANDLE_VALUE)) {}
            ...
            struct read_awaiter {
                ...
                int await_resume() {
                    if (op.error) {
                        ...
                        if (op.error == ERROR_BROKEN_PIPE || op.error == ERROR_PIPE_NOT_CONNECTED)
                            return 0;
            ...
            read_awaiter read(char* buf, size_t len) { return read_awaiter{this, buf, len, {}}; }
            ...
            write_awaiter write(const char* buf, size_t len) { return write_awaiter{this, buf, len, {}}; }
        ...
        inline std::pair<PipeEnd, PipeEnd> pair(size_t buffer = 64 * 1024) {
            ...
            char name[96];
            std::snprintf(name, sizeof(name), "\\\\.\\pipe\\coro_pipe_%lu_%llu", (unsigned long)GetCurrentProcessId(),
                          (unsigned long long)counter.fetch_add(1));
            // 名字是纯 ASCII, 直接加宽
            std::wstring wname(name, name + std::strlen(name));

            // 服务端 = 读端: PIPE_ACCESS_INBOUND (数据只能 客户端→服务端)
            HANDLE server = CreateNamedPipeW(wname.c_str(), PIPE_ACCESS_INBOUND | FILE_FLAG_OVERLAPPED,
            ...
            HANDLE client =
                CreateFileW(wname.c_str(), GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, nullptr);
