...
namespace coro {
    namespace fs {
        ...
        constexpr bool operator&(mode a, mode b) {
        ...
        inline stat_info stat(std::string_view path) {
            ...
            if (!GetFileAttributesExW(detail_fs::utf8_to_wide(path).c_str(), GetFileExInfoStandard, &fa)) {
                io::set_error((int)GetLastError());
        ...
        inline Task<std::string> read_all(std::string_view path) {
            ...
            uint64_t off = 0;
            while (off < st.size) {
                int n = co_await f.read_at(out.data() + off, (size_t)(st.size - off), off);
                if (n < 0) {
                    out.clear();
                    co_return out;
                }
        ...
        /// 返回 false 时错误码在 io::last_error()。
        inline Task<bool> write_all(std::string_view path, std::string_view data, mode m = mode::write) {
            File f = co_await open(path, m);
            if (!f.valid())
                co_return false;
            uint64_t off = 0;
            while (off < data.size()) {
                int n = co_await f.write_at(data.data() + off, (size_t)(data.size() - off), off);
                if (n <= 0)
                    co_return false;
                off += (uint64_t)n;
            }
