...
namespace {
    ...
    struct Timer {
        ...
        explicit Timer(const char* name) : t0(std::chrono::steady_clock::now()) { std::printf("[RUN ] %s\n", name); }
        ~Timer() {
            auto ms =
    ...
    void run_multi_thread() {
        ...
        }
        {
            ...
            for (int i = 0; i < 400000; ++i)
                sched.spawn_any([&done] { return tiny_sleep(1, &done); });
            sched.wait_all();
            std::printf("      完成数: %lld\n", (long long)done.load());
        }
        {
            ...
            for (int i = 0; i < 4000; ++i) {
                sched.spawn_any([&sched, &per_worker] { return count_by_loop(&sched, &per_worker); });
            }
            sched.wait_all();
            std::printf("      各 worker 计数: %lld %lld %lld %lld\n", (long long)per_worker[0].load(),
                        (long long)per_worker[1].load(), (long long)per_worker[2].load(),
                        (long long)per_worker[3].load());
        }
    }
