...
namespace {
    ...
    coro::Task<> registry_scenario(size_t* count_during, bool* current_valid, size_t* count_after) {
...
TEST(RegistryTest, QueueJoinWaitsForAllTasks) {
    ...
    test_util::run_task([&] { return join_scenario(&sum, &join_returned, &unfinished); });
...
TEST(RegistryTest, ConditionWaitNotify) {
    int observed = 0;
    test_util::run_task([&] { return condition_scenario(&observed); });
    EXPECT_EQ(observed, 1);
}

TEST(RegistryTest, ConditionNotifyAll) {
    int woken = 0;
    test_util::run_task([&] { return notify_all_scenario(&woken); });
    EXPECT_EQ(woken, 3);
}
...
TEST(RegistryTest, ActiveTaskCountAndCurrentTask) {
    ...
    test_util::run_task([&] { return registry_scenario(&count_during, &current_valid, &count_after); });
...
TEST(RegistryTest, ConditionCancellationKeepsLockSemantics) {
    ...
    test_util::run_task([&] { return condition_cancel_scenario(&cleaned, &lock_free_after); });
