...
namespace {
    ...
    coro::Task<> void_blocking(bool* done) {
        co_await coro::to_thread([] { std::this_thread::sleep_for(10ms); });
        *done = true;
    }
    ...
    coro::Task<> concurrent_threads(int* sum) {
        auto [a, b, c] = co_await coro::gather(coro::to_thread([] {
    ...
    coro::Task<> loop_keeps_running(bool* tick_ran, int* result) {
        ...
        coro::call_later(10ms, [tick_ran] { *tick_ran = true; });
...
TEST(ThreadTest, ToThreadReturnsResult) {
    int out = 0;
    test_util::run_task([&] { return single_blocking(&out); });
    EXPECT_EQ(out, 42);
}

TEST(ThreadTest, ToThreadVoid) {
    bool done = false;
    test_util::run_task([&] { return void_blocking(&done); });
    EXPECT_TRUE(done);
}

TEST(ThreadTest, ExceptionPropagatesAcrossThreads) {
    bool caught = false;
    test_util::run_task([&] { return exception_from_thread(&caught); });
    EXPECT_TRUE(caught);
}

TEST(ThreadTest, ConcurrentToThreadWithGather) {
    int sum = 0;
    test_util::run_task([&] { return concurrent_threads(&sum); });
    EXPECT_EQ(sum, 60);
}
...
TEST(ThreadTest, EventLoopKeepsRunningWhileWaiting) {
    bool tick_ran = false;
    int result = 0;
    test_util::run_task([&] { return loop_keeps_running(&tick_ran, &result); });
