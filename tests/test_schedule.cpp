...
namespace {
    ...
    coro::Task<> schedule_scenario() {
        ...
        coro::call_soon([] { plain_cb_append(1); });
        coro::call_later(30ms, [] { plain_cb_append(2); });
        coro::call_at(std::chrono::steady_clock::now() + 60ms, [] { plain_cb_append(3); });
    ...
    coro::Task<> timing_scenario(int* at30ms) {
        g_fired = 0;
        coro::call_later(50ms, [] { g_fired = 1; });
...
TEST(ScheduleTest, CallOrderAndCoroutineCallback) {
    test_util::run_task([] { return schedule_scenario(); });
...
TEST(ScheduleTest, CallLaterNotEarly) {
    int at30ms = -1;
    test_util::run_task([&] { return timing_scenario(&at30ms); });
