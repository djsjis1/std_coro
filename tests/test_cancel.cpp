...
TEST(CancelTest, CancelSuspendedTask)
{
    ...
    test_util::run_task([&]
                        { return cancel_and_await(&caught, &cleaned); });
...
TEST(CancelTest, CancelInfiniteLoop)
{
    ...
    int iterations = 0;
    test_util::run_task([&]
                        { return cancel_loop(&caught, &iterations); });
    EXPECT_TRUE(caught);
...
TEST(CancelTest, CancelUnstartedTask)
{
    bool caught = false;
    test_util::run_task([&]
                        { return cancel_unstarted(&caught); });
    EXPECT_TRUE(caught);
}
...
TEST(CancelTest, CancelFinishedTaskIsNoop)
{
    ...
    bool caught = false;
    test_util::run_task([&]
                        { return cancel_finished(&result, &caught); });
    EXPECT_EQ(result, 7);
...
TEST(CancelTest, CancellationShield)
{
    ...
    test_util::run_task([&]
                        { return cancel_shielded(&saw_cancel, &got_error); });
