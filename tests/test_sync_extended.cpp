...
TEST(SyncExtendedTest, ConditionNotify) {
    int woken = 0;
    test_util::run_task([&] { return cond_notify_scenario(&woken); });
    EXPECT_EQ(woken, 1);
}
...
TEST(SyncExtendedTest, ConditionNotifyAll) {
    int final_count = 0;
    test_util::run_task([&] { return cond_notify_all_scenario(&final_count); });
...
TEST(SyncExtendedTest, ConditionPredicateWait) {
    int result = 0;
    test_util::run_task([&] { return cond_predicate_scenario(&result); });
    EXPECT_EQ(result, 42);
}
...
TEST(SyncExtendedTest, QueueTaskDoneAndJoin) {
    ...
    test_util::run_task([&] { return queue_join_scenario(&sum, &join_done); });
    EXPECT_EQ(sum, 15);
...
TEST(SyncExtendedTest, QueueJoinEmpty) {
    bool join_done = false;
    test_util::run_task([&] { return queue_join_empty(&join_done); });
    EXPECT_TRUE(join_done);
}
...
TEST(SyncExtendedTest, QueueStatusQueries) {
    ...
    test_util::run_task([&] { return queue_status(&size_after, &empty_after, &full_after); });
...
TEST(SyncExtendedTest, EventAlreadySetSkipsWait) {
    int out = 0;
    test_util::run_task([&] { return event_already_set(&out); });
    EXPECT_EQ(out, 1);
}
...
TEST(SyncExtendedTest, EventClearCausesWait) {
    ...
    test_util::run_task([&] { return event_clear_and_wait(&timed_out); });
...
TEST(SyncExtendedTest, EventIsSet) {
    ...
    test_util::run_task([&] { return event_is_set(&before, &after); });
    EXPECT_FALSE(before);
    EXPECT_TRUE(after);
}
...
TEST(SyncExtendedTest, TaskDetachRunsToCompletion) {
    ...
    test_util::run_task([&] { return detach_scenario(&counter); });
...
TEST(EventLoopTest, CrossThreadDispatch) {
    ...
    test_util::run_task([&counter, &main_loop]() -> coro::Task<> {
        ...
        std::thread t([main_loop, &counter] { main_loop->dispatch([&counter] { counter.fetch_add(1); }); });
        // 等待投递完成
...
TEST(SyncExtendedTest, EventMultipleWaiters) {
    int final_count = 0;
    test_util::run_task([&] { return event_multi_wait_scenario(&final_count); });
    EXPECT_EQ(final_count, 3);
}
...
TEST(SyncExtendedTest, LockIsLocked) {
    ...
    test_util::run_task([&] { return lock_is_locked(&before, &during, &after); });
...
TEST(SyncExtendedTest, SemaphoreAvailable) {
    ...
    test_util::run_task([&] { return sem_available(&initial, &during, &after); });
...
TEST(SyncExtendedTest, QueueUnfinishedCount) {
    ...
    test_util::run_task([&] { return queue_unfinished(&before_done, &after_done); });
...
TEST(SyncExtendedTest, ConditionWaiterCount) {
    size_t count = 0;
    test_util::run_task([&] { return cond_waiter_count(&count); });
...
TEST(EventLoopTest, ActiveTaskCount) {
    ...
    test_util::run_task([&] { return active_count_scenario(&during, &after); });
...
TEST(SyncExtendedTest, TaskStatusQueries) {
    ...
    test_util::run_task([&] { return task_status_scenario(&started_before, &ready_before, &ready_after); });
