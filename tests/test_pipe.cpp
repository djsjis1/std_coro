...
namespace
{
    ...
    coro::Task<> writer_fills(coro::pipe::PipeEnd wr, const std::string *data,
                             int *total_written, bool *)
    {
        size_t off = 0;
        while (off < data->size())
        {
    ...
    coro::Task<> backpressure_case(const std::string *data, std::string *read_back,
                                   int *total_written, bool *saw_eof)
    {
...
TEST(PipeTest, BackpressureAndEof)
{
    ...
    bool saw_eof = false;
    test_util::run_task([&]
                        { return backpressure_case(&data, &read_back, &total_written, &saw_eof); });
