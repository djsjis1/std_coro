...
namespace
{
    ...
    coro::Task<> collect_until_done(coro::fs::DirectoryWatcher *w,
                                    std::vector<coro::fs::watch_event> *out)
    {
        while (true)
        {
    ...
    coro::Task<> do_ops(const std::string *dir)
    {
        ...
        co_await coro::to_thread([dir]
                                 { std::filesystem::rename(*dir + "/a.txt", *dir + "/b.txt"); });
        co_await coro::sleep(100ms);
        co_await coro::to_thread([dir]
...
TEST(FsWatchTest, CreateModifyRenameRemoveEvents)
{
    ...
    EXPECT_TRUE(has(coro::fs::watch_event_type::created, "a.txt") ||
                has(coro::fs::watch_event_type::modified, "a.txt"))
        << "未见 a.txt 的创建/修改事件";
    // 重命名 a.txt → b.txt
    bool saw_rename = false;
    for (const auto &e : events)
        if (e.type == coro::fs::watch_event_type::renamed &&
            e.path.find("b.txt") != std::string::npos &&
            e.old_path.find("a.txt") != std::string::npos)
            saw_rename = true;
    EXPECT_TRUE(saw_rename) << "未见 a.txt→b.txt 的重命名事件";
    // 删除 b.txt
    EXPECT_TRUE(has(coro::fs::watch_event_type::removed, "b.txt"))
        << "未见 b.txt 的删除事件";
}
